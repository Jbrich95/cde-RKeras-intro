...
Let's switch the problem up a bit. Say I was interest in predicted the probability of having extreme rainfall at this location.
...
I could formulate this problem by transforming $Y$ to a vector of 1s and 0s. This now becomes a graph-level classification problem. We have one value for the entire graph!

```{r, eval=TRUE}
site.ind <-80
Y <- 1*as.matrix(Y[,site.ind]>quantile(Y[,site.ind],prob=0.9))
dim(Y)
dim(X)
...
We will use a global SortPool layer
...
hidden.lay2<- list(hidden.lay1, A2.normed) %>% 
  layer_graph_conv(channels = 16,   activation = "relu")
```
