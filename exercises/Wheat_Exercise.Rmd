...
```
* The dataset contains both training and validation data, with response denoted by $Y$ and predictors denoted by $\mathbf{X}$
* The training and validation sets were determined by randomly allocating approximately 10% of the simulations to the validation set and the remaining 90% to the training set.

```{r, eval=TRUE}
dim(trainData_X)
dim(validationData_X)
```
...
```{r, eval=TRUE}
trainData_X_scaled <- t(apply(trainData_X, 1, 
        rescaleCols,  colMinsX, colMaxsX))
validationData_X_scaled <- t(apply(validationData_X, 1, 
        rescaleCols, colMinsX, colMaxsX))
```
...
trainData_Y = matrix(trainData_Y[, "wheatTotalWeight"], 
                     ncol = 1)
validationData_Y = matrix(
  validationData_Y[, "wheatTotalWeight"],
                    ncol = 1)
```
...
history <- model %>% fit(
  ...
  validation_data = list(
    validationData_X_scaled, validationData_Y)
)
```
...
* Check predictions
```{r, eval=FALSE}
pred_validation <- predict(model, validationData_X_scaled)
plot(validationData_Y, pred_validation, 
...
```{r, eval=TRUE}
yhat <- predict(model2, validationData_X_scaled)
mu <- yhat[, 1]
sigma <- exp(yhat[, 2])
...
# Predictions
```{r, eval=TRUE, echo=FALSE}
plot(validationData_Y, pred_mean, xlab = " True Plant Biomass (kg/ha)",
...
points(x = validationData_Y[plotIndex], y = 0, col = "red")
```
...
* There's some extra details in keras_build.html, including how to save/load models (can be difficult with custom loss functions), multi input/output models, homogeneous model parameters 
